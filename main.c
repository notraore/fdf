...
// void		refill(int sck, t_mlx, t_cor, int *taille)
// {
...
void		ft_fill_tab(int **sck, t_mlx *ptr, t_pts pts, int *taille)
{
	...
	while (sck[pts.j] && sck[pts.j] + 1 != NULL)
	{
		...
		{
			x = pts.j > 0 ? sck[pts.j - 1][pts.i] : 0;
			z = sck[pts.j][pts.i] * 2;
			pts.nx = (pts.x + LEN) - (sck[pts.j][pts.i + 1] * 2);
			pts.ny = pts.y - (sck[pts.j][pts.i + 1] * 2);
			ptr->clr = (sck[pts.j][pts.i] == 0 && (pts.nx == pts.x + LEN)) ? BLUE : GREEN;
			if (pts.i != taille[pts.j] - 1)
				ft_line(pts.x - (z), pts.y - (z), pts.nx, pts.ny, ptr);
			if (pts.j >= 1 && sck[pts.j - 1][pts.i] != 0)
			{
				ptr->y = (sck[pts.j][pts.i] == 0) ? (x * 4) / 2 : (x * 4);
				if (sck[pts.j][pts.i] == 0 && (ptr->clr = GREEN))
					ft_line(pts.x + z, pts.y + z, pts.x - ptr->y, pts.y - LEN - (x * 2), ptr);
				if (sck[pts.j][pts.i] != 0)
					ft_line2(pts.x, pts.y - LEN, pts.x + (x - (sck[pts.j][pts.i])) * 2,
					pts.y + (sck[pts.j - 1][pts.i] - sck[pts.j][pts.i]) * 2, ptr);
			}
			else if (pts.j > 0 && ((sck[pts.j][pts.i] == 0 &&
			(ptr->clr = BLUE)) || sck[pts.j][pts.i] != 0))
				ft_line(pts.x - (z), pts.y - (z), pts.x, pts.y - LEN, ptr);
			pts.x += LEN;
			pts.i += 1;
		}
		pts.y += LEN;
